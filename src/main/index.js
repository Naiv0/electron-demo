...
async function makePartner(event, partner) {
  ...
  try {
    ...
    console.log(e)
    dialog.showErrorBox('Ошибка', "Такой партнер уже существует")
  }
}
...
async function updatePartner(event, partner) {
  ...
  try {
    ...
    dialog.showMessageBox({ message: 'Данные успешно обновлены' })
    return;
  } catch (e) {
    dialog.showErrorBox('Невозможно создать пользователя', 'Такой пользователь уже существует')
    return ('error')
  }
}
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('makePartner', makePartner)
